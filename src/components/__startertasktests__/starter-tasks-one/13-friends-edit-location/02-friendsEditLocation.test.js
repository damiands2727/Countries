...
const generalErrors = {
  general: {
    ...
    hint:
      "HINT: double check for errors on your code. Make sure you are correctly importing " +
  ...
  addFriend: {
    ...
    hint:
      "HINT: double check your addFriend method on your service file and make sure " +
...
const testFriend = {
  ...
  entityTypeId: 1,
  statusId: "Active",
  slug: "string12023-07-13T22:29:08.338Z0",
  skills: null,
  primaryImage: {
    id: 48204,
    entityId: 65502,
    imageTypeId: "Main",
    imageUrl: "https://sabio.la/images/logo.png",
  },
  dateCreated: "2023-07-13T22:29:08.8966667",
  dateModified: "2023-07-13T22:29:08.8966667",
};
...
describe("Friends useNavigate Test", () => {
  ...
  beforeAll(async () => {
    await renderAct(async () => {
      ...
      const addFriend = friendsService.default
        ? friendsService.default.addFriend
  ...
  it(`${
  ...
  } - Friend.jsx component should import useNavigate hook.`, async () => {
    ...
    try {
      ...
      if (
        file?.slice(importIdx - 2, importIdx) === "//" ||
        file?.slice(importIdx - 2, importIdx) === "/*"
      ) {
        hint =
          "HINT: your useNavigate import statement might be commented out.";
      } else if (
        !file
          .slice(importIdx)
          .slice(openCurlIdx, closeCurlIdx)
          .includes("useNavigate")
      ) {
        hint =
          "HINT: make sure you are importing useNavigate from react-router-dom.";
      }
      if (hint) {
  ...
  it(`${
    ...
    const expectedMsg =
      "FriendForm.jsx component should receive location data when navigating from edit button.";
    let actualMsg = expectedMsg;
    let hint = "";
    let friendsComponents,
      ...
      friendCardInstance,
      friendCardComp,
      editBtnInFriendInstance;

    const getLocationInNavigateHook = (context) => {
      ...
      if (
        ...
        "navigator" in memoizedValue
      ) {
        return memoizedValue.navigator.location;
      }
      return getLocationInNavigateHook(context?.next);
    };
    ...
    try {
      ...
      await renderAct(() => {
        friendCardInstance = create(
          <MemoryRouter>
            <Friend friend={testFriend} />
          </MemoryRouter>
        );
      });

      try {
        editBtnInFriendInstance = friendCardInstance?.root?.findAll(
          (node) => {
      ...
      await renderAct(() => {
        editBtnInFriendInstance[0].props.onClick({ preventDefault: () => {} });
        return new Promise((resolve) => setTimeout(resolve, 100));
      });

      friendCardComp = friendCardInstance.root.find(
        starterHelper.elementByType(Friend)
      );
      console.log(friendCardComp);

      const locationObj = getLocationInNavigateHook(
        friendCardComp?._fiber?.dependencies?.firstContext
      );

      if (!!locationObj?.state) {
        const { payload, type } = locationObj.state;
        payload.primaryImage = payload?.primaryImage?.imageUrl
          ? payload.primaryImage.imageUrl
          : "";
        if (!!!payload || !!!type) {
          actualMsg = "Location object is missing one or more properties.";
          hint =
            "HINT: make sure the state in your location object has payload and type " +
            "properties with correct values.";
        } else if (type !== "FRIEND_VIEW") {
          actualMsg = "Location object has incorrect type property.";
          hint =
            "HINT: make sure the type property in your location object has the value 'FRIEND_VIEW'.";
        } else if (
          !starterHelper.deepEqual(
            Object.keys(payload),
            Object.keys(testFriend)
          ) ||
          !!!parseInt(payload?.id)
        ) {
          actualMsg =
            "One or more properties of the payload in the location object are incorrect.";
          hint =
            "HINT: make sure the payload you are passing to the location object is the friend data " +
            "from the friend card that was clicked on.";
        }
      } else {
        actualMsg = "FriendForm.jsx component did not receive location data.";
        hint =
          "HINT: make sure you are passing a location object when you navigate to the FriendForm.jsx " +
      ...
      friendsInstance = appComponent?.root?.find(
        starterHelper.elementByType(Friends)
      );
      friendsComponents = starterHelper.getStatePropVal(
      ...
      if (!!!friendsComponents || friendsComponents.length === 0) {
        ...
        hint =
          "HINT: make sure your Friends.jsx component has the property friendsComponents on state " +
          "with the rendered cards.";
      } else {
        componentsAreFriend = friendsComponents.every(
          starterHelper.elementByType(Friend)
        );
        if (!componentsAreFriend) {
          actualMsg =
            "One or more of the elements in the friendsComponents array are not " +
            "of type Friend.jsx.";
          hint =
            "HINT: make sure you are mapping your friends array and your mapping function " +
            "is returning a Friend.jsx component.";
        } else {
          const editBtns = friendsInstance.findAll(
            (node) => {
              const children = node.children;
              return (
                children.length === 1 &&
          ...
          if (editBtns.length > 0) {
            ...
            if (friendData?.id) {
              ...
              const friendFormInstance = appComponent?.root?.find(
              ...
              if (!friendFormInstance) {
                ...
                hint =
                  "HINT: make sure you are navigating to the /friends/:id url when edit button " +
                  "is clicked.";
              }
            } else {
              actualMsg = "No friend data found in your Friend.jsx component.";
              hint =
                "HINT: make sure you are passing a 'friend' property to your Friend.jsx component " +
                "with the data for each friend in your friendArray.";
            }
          } else {
            actualMsg = "No edit button found.";
            hint =
              "HINT: make sure you are rendering an edit button in your Friend.jsx component.";
          }
        }
      }
    } catch (error) {
...
describe("Friends useLocation Test", () => {
  ...
  it(`${
    ...
    const expectedMsg =
      "Friend Form component should not make a call to " +
    ...
    try {
      ...
      if (friendsService.default) {
        ...
        hint =
          "HINT: make sure you are exporting the getById method in your service file.";
      ...
      let axiosReqInterceptor = axios.interceptors.request.use((config) => {
        if (
          ...
          !isNaN(config.url.split(endpoint)[1])
        ) {
          getByIdReqCount++;
        }
        return config;
      ...
      await renderAct(async () => {
        create(
          <MemoryRouter
            initialEntries={[{ pathname: `/friends/${payload.id}`, state }]}
          >
            <App />
          </MemoryRouter>
        );
      ...
      if (getByIdCalls > 0 || getByIdReqCount > 0) {
        hint =
          "HINT: Make sure you are you are using useLocation hook " +
          "and you don't make a getById service call when friend data is provided.";
        if (getByIdCalls > 0) {
          actualMsg = `getById was called ${
            getByIdCalls === 1 ? "once" : `${getByIdCalls} times`
          } when rendering Friend Form.`;
        } else {
          actualMsg = `${getByIdReqCount} ${
            getByIdReqCount === 1 ? "was" : "were"
          } made to the get friend by id endpoint when rendering Friend Form.`;
        }
      }
    } catch (error) {
      if (actualMsg === expectedMsg) {
        actualMsg =
          "Something went wrong while testing the FriendForm.jsx component.";
  ...
  it(`${
    ...
    const expectedMsg =
      "FriendForm.jsx state properly updates when the data is passed through location.";
    ...
    try {
      ...
      for (const key of Object.keys(payload)) {
        if (
          (key !== "primaryImage" && formState[key] === payload[key]) ||
          (key === "primaryImage" && formState[key] === payload[key]?.imageUrl)
        ) {
          actualMsg =
            "FriendForm.jsx state should not update its state when friend data is passed through " +
            "location state with the incorrect location type.";
          hint =
            "HINT: Make sure you are checking that the 'type' property of the location object has " +
      ...
      formInstance = formComponent.root.find(
        starterHelper.elementByType(FriendForm)
      );
      ...
      for (const key of Object.keys(payload)) {
        if (
          ...
          actualMsg =
            "FriendForm.jsx state should properly update its state when friend data is passed through " +
            "location state.";
          hint =
            "HINT: Make sure you are using the useLocation hook. The location object have the properties " +
            "'payload' and 'type', the first one being a friend object and the second one having the value 'FRIEND_VIEW'.";
          if (key === "primaryImage") {
            hint +=
              " Remember to unpack the imageUrl property from the primaryImage object for your form input.";
          }
          hint +=
