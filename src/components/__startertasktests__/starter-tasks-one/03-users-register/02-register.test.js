...
let mockUser = {
  firstName: "TestUser",
  lastName: "TestUser",
  email: randomNumber + "@" + randomNumber + ".com",
...
axios.interceptors.request.use((config) => {
  ...
  if (config.url.includes("register")) {
    ...
    if (checkingData) {
      requestErrors +=
        "The Payload object that you are passing to your 'register' method is empty.";
    } else {
      for (let index = 0; index < Object.keys(data).length; index++) {
        const element = Object.keys(mockUser)[index];
...
describe("Register component", () => {
  it(`${
  ...
  } - The 'Register' component is a functional component and renders on the DOM`, async () => {
    let actualMsg,
      component,
    ...
    try {
      ...
      let registerInstance = component.root.find(
        starterHelper.elementByType(Register)
      );

      let isClassComp =
        registerInstance._fiber.elementType.toString().split(" ")[0] ===
        "class";

      if (isClassComp) {
        actualMsg = `You are using a 'Class' component and you should be using a 'Functional' component for Register.jsx.`;
      } else {
        actualMsg = expectedMsg;
      }
    } catch (error) {
      actualMsg =
        "The Register component does not exist, or is not rendering on the DOM.";
      hint =
  ...
  it(`${
    number < 10 ? `0${number}` : number
  } - Register.jsx has a route in App.jsx`, async () => {
    let componentInstance, appInstance;
    let hint = "";
    let actualMsg,
      expectedMsg = `The Register Component exists. And there is a route for it in App.jsx`;
    actualMsg = expectedMsg;
    ...
    if (hint) {
      ...
      try {
        ...
        hint =
          "HINT: Make sure you create a route with path: '/register' for Register.jsx in App.jsx.";
      }

      if (!componentInstance) {
        actualMsg = `The Register Component does not exist, or you don't have a route for it in App.jsx`;
      }
    }
  ...
  it(`${
    ...
    let actualMsg,
      component,
      expectedMsg = `You have a form element in your Register Component`;
    let hint = "";
    ...
    try {
      ...
      actualMsg = `You don't have a form element in your Register Component`;
  ...
  it(`${
    number < 10 ? `0${number}` : number
  }  - useState hook is being implemented in Register.jsx`, async () => {
    ...
    if (usingUseState) {
      ...
      actualMsg = "'useState' hook was not found in Regsiter.jsx";
  ...
  it(`${
    number < 10 ? `0${number}` : number
  } - Register.jsx has inputs with names that match the state properties.`, async () => {
    let inputsToFind = [
      "firstName",
      "lastName",
      "email",
    ...
    let actualMsg,
      component,
      expectedMsg = `The Register.jsx has inputs with names that match the state properties.`;
    let hint = "";
    try {
      await renderAct(() => {
        component = renderer.create(
    ...
    if (hint) {
      actualMsg =
        "The Register.jsx does not have inputs with names that match the state properties.";
    }
  ...
  it(`${
    number < 10 ? `0${number}` : number
  } - Register.jsx should have a state object and have properties matching Api documentation`, async () => {
    let formInstance;
    let component;
    let renderError = "";
    const expectedMsg =
    ...
    const registerPropKeys = Object.keys(registerProperties);
    registerPropKeys.push("tenantId");
    ...
    formInstance = component?.root?.find(starterHelper.elementByType(Register));

    try {
      hasAllProperties = registerPropKeys.every((key) => {
        const valType =
          registerProperties[key] === "number" ? "number" : "string";
    ...
    if (!hasAllProperties) {
      actualMsg =
        "Register component is missing a state or some of its properties do not match a register object." +
        renderError;
    }
  ...
  it(`${
    number < 10 ? `0${number}` : number
  } - You have a 'Register' button type of 'Submit' on your component.`, async () => {
    let button;
    let hint = "";
    let actualMsg,
      component,
      expectedMsg = `You have a 'Register' button type of "Submit"`;
    ...
    try {
      ...
      if (button.length === 1) {
        ...
        actualMsg = `You do not have a 'Register' button  type of "Submit" or you have more than 1 button type "Submit"`;
        hint = `HINT: Is your 'Register' button type of "Submit" and Do you have just ONE button type of "Submit" in your Register Form?`;
      }
    } catch (e) {
      actualMsg = `You do not have a 'Register' button type of "Submit" or you have more than 1 button type "Submit"`;
      hint = `HINT: Is your 'Register' button type of "Submit" and Do you have just ONE button type of "Submit" in your Register Form?`;
  ...
  it(`${
    number < 10 ? `0${number}` : number
  } - 'Register' button has an onClick handler function`, async () => {
    ...
    if (buttonTest) {
      ...
      try {
        if (clickHandler) {
          ...
          hint = `HINT: do you have an onClick property in your 'Register' button?`;
        }
      } catch (e) {
        actualMsg = `Submit button doesn't have an onClick handler`;
        hint = `HINT: do you have an onClick property in your 'Register' button?`;
...
describe("Register form functionality", () => {
  ...
  beforeAll(async () => {
    ...
    formInstance = appComponent.root.find(
      starterHelper.elementByType(Register)
    );
  });
  ...
  it(`${
    ...
    if (!inputs) {
      ...
      hint =
        "HINT: there should be 6 inputs on your component, one for each property in your state, except the tenantId";
    }
    ...
    await renderer.act(async () => {
      for (const prop of registerPropKeys) {
    ...
    if (regsiterError) {
      actualMsg =
        "The usersService.js file is not exporting the register method";
      hint = "HINT: Make sure you are exporting the 'register' method";
    }
    expect(actualMsg).sabioToBe(expectedMsg, hint);
  });
  number++;
  ...
  it(`${
    number < 10 ? `0${number}` : number
  }: When clicking on the 'Register' button the register method gets invoked with the proper payload and you get the right response`, async () => {
    let actualMsg;
    let expectedMsg = "Expected 'register' to have been called successfully";
    actualMsg = expectedMsg;
    let hint = "";

    try {
      let registerAxiosSpy = jest.spyOn(usersServiceFile, "register");
      ...
      if (registerAxiosSpy.mock.calls.length === 0) {
        actualMsg = `Submit button doesn't have an onClick handler or you have more than ONE button type of "Submit"`;
        hint = `HINT: do you have just ONE button type of "Submit" and an onClick handler in your 'Register' button?`;
      }
    } catch (e) {
      if (e.message === "Response status is not 200") {
        actualMsg = "'register' service was called but it was not successful.";
  ...
  it(`${
    number < 10 ? `0${number}` : number
  } - You have a 'register' service method with the right config and you are passing the correct payload`, async () => {
    let actualMsg,
      expectedMsg =
        "Expected register to have been called successfully with the correct parameters.";

    let hint = "";

    try {
      jest.spyOn(usersServiceFile, "register");

      if (registerErrors) {
        actualMsg = registerErrors;
        hint = "HINT: Check the config object on your register method.";
      } else {
        actualMsg = expectedMsg;
      }
    } catch (e) {
  ...
  it(`${
    number < 10 ? `0${number}` : number
  } - toastr.success should be invoked on successful register`, async () => {
    let expectedMsg =
      "Expected toastr.success to have been called when clicking on the submit button and the register is successful";
    let actualMsg, hint;

    try {
      const spytoastr = jest.spyOn(toastr, "success");

      jest.spyOn(usersServiceFile, "register").mockResolvedValue({});

      const submitButton = appComponent.root.findByProps({ type: "submit" });

      const registerResponse = { success: true };
      usersServiceFile.register.mockResolvedValueOnce({ registerResponse });
      await renderAct(async () => {
      ...
      actualMsg =
        "there was an error invoking toastr.success when clicking on the submit button and the register is successful";
  ...
  it(`${
    number < 10 ? `0${number}` : number
  } - toastr.error should be invoked on a failed register`, async () => {
    let expectedMsg =
      "toastr.error to have been called when clicking the submit button and the register failed";
    let hint, actualMsg;

    try {
      const spytoastr = jest.spyOn(toastr, "error");

      jest.spyOn(usersServiceFile, "register").mockRejectedValue({});

      const submitButton = appComponent.root.findByProps({ type: "submit" });

      const registerResponse = { success: true };
      usersServiceFile.register.mockRejectedValueOnce({
      ...
      actualMsg =
        "toastr.error was not invoked when clicking on the submit button and the register failed.";
